...
extern void realme_get_battery_callback_register(bool (*handler)(u8 battery_type, u8 *value));
extern void realme_event_notify(uint8_t event_type, uint8_t *data, uint16_t len);
extern void realme_multi_info_set(uint8_t *data, uint8_t len, uint8_t num);
extern void realme_is_tws_master_callback_register(bool (*handler)(void));
extern void realme_message_callback_register(int (*handler)(int id, int opcode, u8 *data, u32 len));
extern int realme_tws_data_deal(u8 *data, int len);
extern int get_realme_connect_state(void);
extern void feature_id_state_commit(uint8_t id_type);
extern void realme_product_test_set(uint8_t enable, int (*func)(uint8_t *buf, uint16_t len));
extern int realme_product_test_spp_send(uint8_t *data, uint16_t len);
extern void realme_action_before_ota_reboot_callback_register(int (*handler)(void *priv));
extern void realme_action_before_ota_start_callback_register(int (*handler)(void *priv));
