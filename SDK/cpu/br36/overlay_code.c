...
static u32 audio_overlay_used_type = -1;

void overlay_load_code(u32 type)
{
    int i = 0;
    if (type == OVERLAY_M4A || type == OVERLAY_AEC) {	//AEC/AAC复用同一块OVERLAY
        if (audio_overlay_used_type == type) {
            return;
        }
        audio_overlay_used_type = type;
    }
