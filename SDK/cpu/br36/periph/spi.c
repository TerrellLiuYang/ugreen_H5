...
static void spi1_iomc_config(spi_dev spi)
{
    ...
    gpio_set_function(port[0] / 16, BIT(port[0] % 16), PORT_FUNC_SPI1_CLK);
    gpio_set_function(port[1] / 16, BIT(port[1] % 16), PORT_FUNC_SPI1_DA0);
    gpio_set_function(port[2] / 16, BIT(port[2] % 16), PORT_FUNC_SPI1_DA1);
    if (mode == SPI_MODE_UNIDIR_4BIT) {
        gpio_set_function(port[3] / 16, BIT(port[3] % 16), PORT_FUNC_SPI1_DA2);
        gpio_set_function(port[4] / 16, BIT(port[4] % 16), PORT_FUNC_SPI1_DA3);
    }
}
...
static void spi2_iomc_config(spi_dev spi)
{
    u8 *port = spi_get_info_port(spi);
    u8 mode = spi_get_info_mode(spi);
    gpio_set_function(port[0] / 16, BIT(port[0] % 16), PORT_FUNC_SPI2_CLK);
    gpio_set_function(port[1] / 16, BIT(port[1] % 16), PORT_FUNC_SPI2_DA0);
    gpio_set_function(port[2] / 16, BIT(port[2] % 16), PORT_FUNC_SPI2_DA1);
    if (mode == SPI_MODE_UNIDIR_4BIT) {
        gpio_set_function(port[3] / 16, BIT(port[3] % 16), PORT_FUNC_SPI2_DA2);
        gpio_set_function(port[4] / 16, BIT(port[4] % 16), PORT_FUNC_SPI2_DA3);
    }
}
