                              ...
                              unsigned char *readBuf)
{
    ...
    delay_nops(sh3001_info->iic_delay);
...
#define spi_cs_init() \
    do { \
        gpio_set_mode(sh3001_info->spi_cs_pin/16,BIT(sh3001_info->spi_cs_pin%16),PORT_OUTPUT_HIGH); \
    } while (0)
#define spi_cs_uninit() \
    do { \
        gpio_set_mode(sh3001_info->spi_cs_pin/16,BIT(sh3001_info->spi_cs_pin%16),PORT_HIGHZ); \
    } while (0)
...
u8 SH3001_sensor_init(void *priv)
{
    ...
    iic_init(sh3001_info->iic_hdl, get_iic_config(sh3001_info->iic_hdl));
#else//spi interface
    spi_cs_init();
    spi_init();
#endif
//int module io init
    gpio_set_mode(IO_PORT_SPILT(sh3001_int_pin), PORT_INPUT_FLOATING); //PORT_INPUT_PULLUP_10K,PORT_INPUT_PULLDOWN_10K
    gpio_set_mode(IO_PORT_SPILT(SH3001_INT_IO2), PORT_INPUT_FLOATING); //PORT_INPUT_PULLUP_10K,PORT_INPUT_PULLDOWN_10K
    return SH3001_init();
}
