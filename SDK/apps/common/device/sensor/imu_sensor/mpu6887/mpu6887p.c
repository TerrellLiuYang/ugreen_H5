                             ...
                             u16 readLen)
{
    ...
    delay_nops(mpu6887p_info->iic_delay);
...
#define spi_cs_init() \
    do { \
        gpio_set_mode(mpu6887p_info->spi_cs_pin/16,BIT(mpu6887p_info->spi_cs_pin%16),PORT_OUTPUT_HIGH); \
    } while (0)
#define spi_cs_uninit() \
    do { \
        gpio_set_mode(mpu6887p_info->spi_cs_pin/16,BIT(mpu6887p_info->spi_cs_pin%16),PORT_HIGHZ); \
    } while (0)
...
u8 mpu6887p_sensor_init(void *priv)
{
    ...
    iic_init(mpu6887p_info->iic_hdl, get_iic_config(mpu6887p_info->iic_hdl));
#elif (MPU6887P_USER_INTERFACE==MPU6887P_USE_SPI)//spi interface
    spi_cs_init();
    spi_init();
#endif
//int module io init
    gpio_set_mode(IO_PORT_SPILT(mpu6887_int_pin), PORT_INPUT_FLOATING); //PORT_INPUT_PULLUP_10K,PORT_INPUT_PULLDOWN_10K
