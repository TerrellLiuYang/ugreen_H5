...
u8 imusensor_write_nbyte(u8 w_chip_id, u8 register_address, u8 *buf, u8 data_len)
{
    ...
    delay_nops(imuSensor_info->iic_delay);
...
int imu_sensor_io_ctl(char *sensor_name, enum OPERATION_SENSOR cmd, void *arg)
{
    int retval = -1;
    list_for_each_imusensor(imuSensor_hdl) {
        ...
        if (!memcmp(imuSensor_hdl->logo, sensor_name, strlen(imuSensor_hdl->logo))) {
            ...
            if (((u32)strrchr((const char *)sensor_name, '\0') - (u32)sensor_name) != ((u32)strrchr(imuSensor_hdl->logo, '\0') - (u32)(imuSensor_hdl->logo))) {
                goto __imu_exit;
            }
...
int imu_sensor_init(void *_data, u16 _data_size)
{
    ...
    platform_data = (struct imusensor_platform_data *)_data;
...
u8 test_buf[2024]; //(8+12+12+4)*60=
void imu_test()
{
    char name_test[20] = "sh3011";
    u8 arg_data;
    ...
    memset((u8 *)test_buf, 0, sizeof(test_buf));
    uint64_t timestamp;
    float accel_g[3];
    float gyro_dps[3];
    float temp_degc;
    ...
    while (1) {
        ...
        if (pack_len > 0) {
            memcpy((u8 *)&timestamp, test_buf, 8);//模式：ICM42670P_FIFO_DATA_FIT:0, SCALED_DATA_G_DPS:1
        ...
        /*     mdelay(10); */
        /*     pack_len = imu_sensor_io_ctl(name_test, IMU_GET_SENSOR_READ_FIFO, test_buf); */
        /* log_info("qmi8658 raw:acc_x:%d acc_y:%d acc_z:%d gyro_x:%d gyro_y:%d gyro_z:%d, tim_count:%d", (s16)(test_buf[1]<<8|test_buf[0]), (s16)(test_buf[3]<<8|test_buf[2]), (s16)(test_buf[5]<<8|test_buf[4]), (s16)(test_buf[7]<<8|test_buf[6]), (s16)(test_buf[9]<<8|test_buf[8]), (s16)(test_buf[11]<<8|test_buf[10]),pack_len);//共fifo_level组数据, 只打印第一组数据 */
