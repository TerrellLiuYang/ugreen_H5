...
void delay_nops(volatile u32 t);
void udelay(u32 us);
                              ...
                              u32 readLen)
{
    ...
    delay_nops(icm42670p_info->iic_delay);
...
#define spi_cs_init() \
    do { \
        gpio_set_mode(icm42670p_info->spi_cs_pin/16,BIT(icm42670p_info->spi_cs_pin%16),PORT_OUTPUT_HIGH); \
    } while (0)
#define spi_cs_uninit() \
    do { \
        gpio_set_mode(icm42670p_info->spi_cs_pin/16,BIT(icm42670p_info->spi_cs_pin%16),PORT_HIGHZ); \
    } while (0)
...
{
    u32 rc = 0;
    rc = icm42670p_write(ICM42670P_SLAVE_ADDRESS, reg, wbuffer, wlen);
    if (rc == wlen) {
        return 0;
    } else {
        return -1;
...
//init interrup io, Register the callback function.
    gpio_set_mode(IO_PORT_SPILT(icm42670p_int_pin1), PORT_INPUT_FLOATING); //PORT_INPUT_PULLUP_10K,PORT_INPUT_PULLDOWN_10K
    gpio_set_mode(IO_PORT_SPILT(icm42670p_int_pin2), PORT_INPUT_FLOATING); //PORT_INPUT_PULLUP_10K,PORT_INPUT_PULLDOWN_10K
...
//iic or spi init
    ...
    case UI_SPI4: {
#if (ICM42670P_USER_INTERFACE==ICM42670P_USE_SPI)
        spi_cs_init();
        spi_init();
        /* uint8_t dummy = 0; */
    ...
    case UI_I2C:
        ...
        iic_init(icm42670p_info->iic_hdl, get_iic_config(icm42670p_info->iic_hdl));
#endif
        break;
    default:
        return -1;
...
void imu_callback(inv_imu_sensor_event_t *event)
{
    ...
    */
     ...
     */
    /* log_info("event->sensor_mask:0x%x",event->sensor_mask); */
    if (event->sensor_mask & (1 << INV_SENSOR_ACCEL) && event->sensor_mask & (1 << INV_SENSOR_GYRO)) {
#if ICM42670P_FIFO_DATA_FIT
