...
#define GMA_GMA_SOURCH     GMA_GMA_FROM_FILE
...
int gma_prev_init(void)
{
    ...
    const char *ali_str = ali_para_string[ALI_PARA_ACTIVE_NUM];
    char ali_license[128] = {0};

    //check if exist flash file
    if (read_cfg_file(ali_license, sizeof(ali_license), FLASH_RES_PATH"license.txt") == TRUE) {
        ali_str = ali_license;
    }
...
int gma_bt_tws_event_handler(struct tws_event *bt)
{
    int role = bt->args[0];
...
int gma_bt_status_event_handler(struct bt_event *bt)
{
    return 0;
}
...
#endif
    /* .tws_sync_func = ; */
    .tws_event_handler = gma_bt_tws_event_handler,
    .bt_event_handler = gma_bt_status_event_handler,
};
#endif
