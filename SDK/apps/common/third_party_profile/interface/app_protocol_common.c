...
int app_protocol_tws_event_private_deal(struct tws_event *event)
{
    if (private_handle && private_handle->tws_event_handler) {
        return private_handle->tws_event_handler(event);
    }
    return 0;
}

int app_protocol_bt_event_private_deal(struct bt_event *bt)
{
    if (private_handle && private_handle->bt_event_handler) {
        return private_handle->bt_event_handler(bt);
    }
    return 0;
}
...
int app_protocol_start_speech_by_key(struct sys_event *event)
{
    ...
    if (app_protocol_check_connect_success() && (bt_get_curr_channel_state()&A2DP_CH)) {
        printf("no connect\n");
    } else {
...
{
    u8 vol_l = (vol * app_audio_volume_max_query(AppVol_BT_MUSIC)) / 0x64;
    u8 vol_r = (vol * app_audio_volume_max_query(AppVol_BT_MUSIC)) / 0x64;
...
void app_protocol_post_bt_event(u8 event, void *priv)
{
    ...
    struct bt_event e;
    e.event = event;
    app_protocol_bt_ai_event_handler(&e);
    //测试代码END
#endif
}
...
static int app_protocol_bt_status_event_handler(struct bt_event *bt)
{
    ...
    app_protocol_bt_event_private_deal(bt);
    return 0;
}

static void app_protocol_bt_tws_event_handler(struct tws_event *tws)
{
    int role = tws->args[0];
    ...
    app_protocol_tws_event_private_deal(tws);
}
...
int app_protocol_sys_event_handler(struct sys_event *event)
{
    ...
    switch (event->type) {
    case SYS_BT_EVENT:
        if (((u32)event->arg == SYS_BT_AI_EVENT_TYPE_STATUS)) {
...
static int bt_app_status_event_handler(int *msg)
{
    ...
    app_protocol_bt_status_event_handler(bt);
    return 0;
}
APP_MSG_HANDLER(bt_app_protocol__msg_handler) = {
    .owner      = 0xff,
...
static int bt_app_tws_status_event_handler(int *msg)
{
    ...
    app_protocol_bt_tws_event_handler(evt);
    return 0;
}
APP_MSG_HANDLER(bt_app_tws_msg_handler_stub) = {
    .owner      = 0xff,
    .from       = MSG_FROM_TWS,
