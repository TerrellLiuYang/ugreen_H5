...
static int app_protocol_realme_init(int match_id);
...
struct app_ctrl_operation_t {
    ...
    int(*protocol_exit)(void);
    int(*adv_enable)(u8 enable);
    int(*ibeacon_adv)(u8 sw);
    ...
    int(*send_voice_data)(u8 *buf, u16 len);
...
typedef struct app_protocol {
    ...
    const struct app_protocol_info_t  *settings;
    const struct app_ctrl_operation_t *app_ctrl;
    const struct callback_register_t  *callback;
...
static void app_protocol_tick_resume(void *ptr)
{
...
void app_protocol_init(int handler_id)
{
    ...
    if (__this->running_protocol_number < MAX_NUMBER_RUN) {
        ...
        if (app_protocol_realme_init(handler_id)) {
            __this->running_protocol_number++;
        }
#endif
...
void app_protocol_all_init()
{
    ...
    app_protocol_init(REALME_HANDLER_ID);
...
void app_protocol_exit(int handler_id)
{
    ...
    if (__this->run_flag && (!__this->running_protocol_number)) {
        ...
        os_sem_create(&app_protocol_exit_sem, 0);
        os_taskq_post_type(APP_PROTOCOL_TASK_NAME, APP_PROTOCOL_TASK_EXIT, 1, (int *)&app_protocol_exit_sem);
        os_sem_pend(&app_protocol_exit_sem, 0);
...
u16 app_protocol_send_voice_data(uint8_t *voice_buf, uint16_t voice_len)
{
    int error = 0;
...
#if (BT_AI_SEL_PROTOCOL & REALME_EN)
u8 realme_debug_enable = 0x03; // BIT(0):printf BIT(1):put_buf

extern const u8 sdp_realme_spp_service_data[];
AI_RECORD_REGISTER(realme_record_item) = {
    .service_record = (u8 *)sdp_realme_spp_service_data,
    .service_record_handle = 0x00010038,
};

//u8 realme_tws_support = 1;
static const struct callback_register_t realme_callback = {
    .message_handler_regedit = realme_message_callback_register,
    .check_tws_role_is_master_register = realme_is_tws_master_callback_register,
    .check_some_status_register = NULL,
    .tx_resume = NULL,
    .rx_resume = NULL,
    .get_battery = realme_get_battery_callback_register,
};
static const struct app_ctrl_operation_t realme_ctrl = {
    .protocol_init = realme_all_init,
    .protocol_exit = realme_all_exit,
    .adv_enable    = NULL,
    .send_voice_data = NULL,
    .get_auth_state = get_realme_connect_state,
    .start_voice_recognition = NULL,
    .disconnect   = realme_disconnect,
    .update_tws_state_to_lib = realme_update_tws_state_to_lib,
    .tws_receive_sync_data = realme_tws_data_deal,
};
static const struct app_protocol_info_t realme_settings = {
    .set_product_id = NULL,
...
static char realme_product_id[] = "0000";
static char realme_protocol_version[3] = "001";
static char realme_version[] = "1,2,1.1.0.1,2,2,1.1.0.1,3,2,1.1.0.1";
static uint32_t realme_capabilities_id = BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(7) | BIT(10) | BIT(11);
static uint16_t realme_vid = 0x05D6;
static uint8_t realme_device_id_type = 1; // tws的200006类型这里为1，非tws为0
static uint8_t realme_codec = 1;
static uint8_t realme_color = 1;

static struct __realme_info *realme_info;

static void realme_info_init(void)
{
    realme_info = malloc(sizeof(struct __realme_info));
    memset(realme_info, 0, sizeof(struct __realme_info));
    realme_info->realme_capabilities = realme_capabilities_id;
    realme_info->realme_color_id = realme_color;
    memcpy(realme_info->special_id.product_id, realme_product_id, 4);
    realme_info->vid = realme_vid;
    realme_info->device_id_type = realme_device_id_type;
    realme_info->special_id.protocol_version = realme_protocol_version;
    realme_info->special_id.protocol_version_len = sizeof(realme_protocol_version);
    realme_info->special_id.version = realme_version;
    realme_info->special_id.version_len = strlen(realme_version);
    realme_info->audio_code = realme_codec;
    realme_info_register(realme_info);
}

void realme_hfp_battery_get(u8 *left, u8 *right, u8 *box)
{
    uint8_t battery_value[APP_PROTOCOL_BAT_T_MAX] = {0};
    app_protocol_get_battery(BIT(APP_PROTOCOL_BAT_T_TWS_LEFT), battery_value);
    *left = battery_value[APP_PROTOCOL_BAT_T_TWS_LEFT];

    app_protocol_get_battery(BIT(APP_PROTOCOL_BAT_T_TWS_RIGHT), battery_value);
    *right = battery_value[APP_PROTOCOL_BAT_T_TWS_RIGHT];

    app_protocol_get_battery(BIT(APP_PROTOCOL_BAT_T_BOX), battery_value);
    *box = battery_value[APP_PROTOCOL_BAT_T_BOX];
}
/*
上层状态改变需要发送消息通知手机，使用这个函数
void realme_event_notify(uint8_t event_type, uint8_t *data, uint16_t len)
*/

//lib收到控制消息后，会传消息上来处理
static int realme_special_message(int id, int opcode, u8 *data, u32 len)
{
    ...
    case APP_PROTOCOL_REALME_EQ_SET:
        // 设置eq模式
        uint8_t eq_mode = realme_info->eq_id;
        break;
    case APP_PROTOCOL_REALME_HIGH_VOL_GAIN_SET:
        // 设置音阶
        uint8_t high_vol_gain = realme_info->high_vol_gain_level;
        break;
    ...
    case APP_PROTOCOL_REALME_UTC_TIME_SYNC:
        //UTC时间同步
        uint8_t time = realme_info->time_stamp;
        break;
    case APP_PROTOCOL_REALME_POWER_OFF_TIME_SET:
        //设置自动关机时间
        uint8_t power_off_time = realme_info->auto_power_off_time;
        break;
...
static int app_protocol_realme_init(int match_id)
{
    if (REALME_HANDLER_ID != match_id) {
        return 0;
    }
    APP_PROTOCOL_LOG("app_protocol_realme_init\n");
    app_protocol_interface_t *app = get_interface_for_id(REALME_HANDLER_ID);
    if (app) {
        APP_PROTOCOL_LOG("REALME_HANDLER_ID init again\n");
    } else {
        app = get_empty_interface();
        if (app) {
            app->handler_id = REALME_HANDLER_ID;
            app->loop = NULL;
            app->tx_loop = NULL;
            app->special_message = realme_special_message;
            app->settings = &realme_settings;
            app->app_ctrl = &realme_ctrl;
            app->callback = &realme_callback;
            realme_product_test_set(REALME_PRODUCT_TEST, realme_product_test_recieve_cbk);
            realme_info_init();
            app_protocol_common_init(app);
            APP_PROTOCOL_LOG("REALME_HANDLER_ID init OK\n");
            return 1;
        } else {
            APP_PROTOCOL_LOG("not support more app protocal\n");
        }
...
#if APP_PROTOCOL_GFPS_CODE
u8 gfp_debug_enable = 0x03; // BIT(0):printf BIT(1):put_buf

// 控制GFP广播时间,开广播时过多少时间关广播,为0时不作控制,过认证时不能开
u32 gfps_adv_close_time = 0;//单位：秒
...
static struct callback_register_t gfps_callback = {
...
static struct app_ctrl_operation_t gfps_ctrl = {
...
static struct app_protocol_info_t gfps_settings = {
    .set_product_id = NULL,
    .set_special_info_group = NULL,
};
...
static int app_protocol_gfps_init(int match_id)
{
    ...
    if (app) {
        ...
        if (app) {
            ...
            //gfps_personalized_name_set(gfps_special_name, sizeof(gfps_special_name));
            app_protocol_common_init(app);
