...
extern void qiot_start_report_property_timer();
extern void qiot_stop_report_property_timer();
extern void wdt_clr();
extern void notify_update_connect_parameter(u8 table_index);
...
static ble_qiot_ret_status_t ble_ota_write_data_to_flash(void)
{
    ...
    /* g_printf("%s\n", __func__); */
    /* printf("============>ota data:\n"); */
    /* put_buf(sg_ota_data_buf, sg_ota_data_buf_size); */
    dual_bank_update_write((void *)sg_ota_data_buf, sg_ota_data_buf_size, ble_ota_write_end_callback);
    ...
    if (tws_api_get_tws_state() & TWS_STA_SIBLING_CONNECTED) {
        ...
        if (sg_ota_tws_same_api && sg_ota_tws_same_api->tws_ota_data_send_pend) {
            if (sg_ota_tws_same_api->tws_ota_data_send_pend()) {
                ble_qiot_log_e("qiot tws update pend timeout\n");
                return BLE_QIOT_RS_ERR;
            }
        }
    } else {
        ble_qiot_log_e("TWS DISCONNECT, OTA ABORT!");
        return BLE_QIOT_RS_ERR;
    }
...
static ble_qiot_ret_status_t ble_ota_init(void)
{
    ...
    ble_qiot_log_hex(BLE_QIOT_LOG_LEVEL_INFO, "ota info", (const char *)&sg_ota_info, sizeof(sg_ota_info));
...
{
    BUFF_LEN_SANITY_CHECK(buf_len, (int)sizeof(ble_ota_file_info) - BLE_QIOT_OTA_MAX_VERSION_STR, BLE_QIOT_RS_ERR_PARA);
    ...
    ble_qiot_log_i("request ota, size: %x, crc: %x, version: %s, version_len:%d!\n", file_size, file_crc, p, version_len);
    ble_qiot_log_i("task:%s!\n", os_current_task());
    ...
    if (BLE_OTA_ENABLE == ret) {
        ...
        if (sg_ota_tws_same_api == NULL) {
            ble_qiot_log_i("tws not connect! download reject!\n");
            ret = 0;
        } else {
            if (sg_ota_tws_same_api && sg_ota_tws_same_api->tws_ota_start) {
                ...
                if (sg_ota_tws_same_api->tws_ota_start(&tws_update_para) != 0) {
                    ble_qiot_log_i("tws_ota_start err!\n");
                    ret = 0;
                }
            }
        }
    ...
    if (BLE_OTA_ENABLE == ret) {
        set_ota_status(1);
        dual_bank_passive_update_init(file_crc, file_size, BLE_QIOT_OTA_BUF_SIZE, NULL);
        ble_ota_download_address_set();
        if (dual_bank_update_allow_check(file_size) != 0) {
            ...
            dual_bank_passive_update_exit(NULL);
            set_ota_status(0);
            ble_qiot_log_i("dual_bank_update_allow_check err!\n");
            ret = 0;
        }
...
ble_qiot_ret_status_t ble_ota_file_end_handle(void)
{
    ...
    ble_qiot_log_e("old_sys_clk:%d", old_sys_clk);
    ...
    while (crc_file_len < sg_ota_info.download_file_info.file_size) {
        ...
        memset(sg_ota_data_buf, 0, sizeof(sg_ota_data_buf));
        dual_bank_update_read_data(crc_file_len, (u8 *)sg_ota_data_buf, crc_buf_len);
...
static ble_qiot_ret_status_t ble_qiot_ota_data_saved(char *data, uint16_t data_len)
{
    ...
    if ((data_len + sg_ota_data_buf_size) > sizeof(sg_ota_data_buf)) {
        ...
        /* printf("qiot ota process:%d", percent); */
...
ble_qiot_ret_status_t ble_ota_data_handle(const char *in_buf, int buf_len)
{
    ...
    ble_qiot_log_d("current seq %d, expect seq %d", seq, ble_ota_next_seq_get());
