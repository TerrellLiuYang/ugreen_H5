...
ble_qiot_ret_status_t ble_qiot_advertising_start(uint8_t start)
{
    ...
    if (E_LLSYNC_BIND_IDLE == llsync_bind_state_get()) {
        ...
        if (NULL == sg_bind_timer) {
            sg_bind_timer = ble_timer_create(BLE_TIMER_ONE_SHOT_TYPE, ble_bind_timer_callback, "bind_timer");
            if (NULL == sg_bind_timer) {
        ...
        ble_advertising_start(&my_adv_info, start);
    ...
    if (!BLE_QIOT_IS_SLICE_HEADER(flag)) {
        ...
        if (sg_ble_slice_data.buf_len + (in_len - header_len) > sizeof(sg_ble_slice_data.buf)) {
            ble_qiot_log_e("too long data: %d > %d", sg_ble_slice_data.buf_len + (in_len - header_len),
                           (int)sizeof(sg_ble_slice_data.buf));
            return -1;
        }
    }
...
int ble_device_info_msg_handle(const char *in_buf, int in_len)
{
    ...
    case E_DEV_MSG_IOS_SYSTEM:
        ...
        break;
    case E_DEV_MSG_KEEP_ALIVE:
        ble_keep_alive(p_data + 1, p_data_len - 1);
        out_buf[0] = p_data_len >= 2 ? p_data[1] : 0;  // report keep alive interval
        ret        = ble_event_notify((uint8_t)BLE_QIOT_EVENT_UP_KEEP_ALIVE, NULL, 0, out_buf, 1);
        break;
    default:
