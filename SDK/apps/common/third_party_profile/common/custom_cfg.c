...
#define SDFILE_API_MANIFEST					2
#define USE_FS_API_SEL						SDFILE_API_MANIFEST
...
u32 get_cfg_data_connect_by_name(u8 *name, u8 *data, u16 len, u16 *real_len)
{
    ...
    *real_len = 0;
    ...
    r_len = fread((void *)&cfg_head, sizeof(cfg_head_t), 1, fp);
    if (r_len != sizeof(cfg_head_t)) {
        cfg_puts("file read cfg head fail\n");
        err = CFG_ERR_FILE_READ_ERR;
        goto _ERR_RET;
    }
#elif (USE_FS_API_SEL == SDFILE_API_MANIFEST)
    RESFILE *fp = NULL;
    fp = resfile_open(RES_CUSTOM_CFG_FILE);
    if (!fp) {
        cfg_puts("file open fail");
        err = CFG_ERR_FILE_READ_ERR;
        goto _ERR_RET;
    }
    r_len = resfile_read(fp, (void *)&cfg_head, sizeof(cfg_head_t));
    if (r_len != sizeof(cfg_head_t)) {
        cfg_puts("file read cfg head fail\n");
        err = CFG_ERR_FILE_READ_ERR;
        goto _ERR_RET;
    ...
    cfg_printf("read file req_len:%lx ret_len:%x\n", sizeof(cfg_head_t), cfg_head.len);
    cfg_puts("cfg_head:\n");
    ...
    for (i = 0; i < cfg_head.item_count; i++) {
        ...
        r_len = fread((void *)&cfg_item_head, sizeof(cfg_item_head_t), 1, fp);
...
#elif (USE_FS_API_SEL == SDFILE_API_MANIFEST)
        resfile_seek(fp, offset, SEEK_SET);
        r_len = resfile_read(fp, (void *)&cfg_item_head, sizeof(cfg_item_head_t));
        if (r_len != sizeof(cfg_item_head_t)) {
            cfg_puts("file read cfg item head fail\n");
            err = CFG_ERR_FILE_READ_ERR;
            goto _ERR_RET;
        ...
        if (0 == memcmp(cfg_item_head.name, name, strlen((const char *)name))) {
            ...
            *real_len = cfg_item_head.len;
            ...
            r_len = fread(data, cfg_item_head.len, 1, fp);
...
#elif (USE_FS_API_SEL == SDFILE_API_MANIFEST)
            resfile_seek(fp, offset, SEEK_SET);
            r_len = resfile_read(fp, data, cfg_item_head.len);
            if (r_len != cfg_item_head.len) {
                cfg_puts("read cfg item content fail\n");
                err = CFG_ERR_FILE_READ_ERR;
                goto _ERR_RET;
...
{
#if (USE_FS_API_SEL == SDFILE_API_MANIFEST)
    RESFILE *fp = resfile_open(EXIF_CFG_PATH);
    if (fp) {
