...
//这是一个公共的SPP使用接口层，应该不需要加太多其它具体功能的宏进行编译控制
//transport_spp_init如果在APP case没有被调用，这些代码应该会被自动优化的。
//要使用的时候app case要有对应的初始化位置,
//PS:调用关系：这个文件的函数只能被APP CASE调用，不能调用app case的函数
...
void transport_spp_disconnect(void)
{
    if (SPP_USER_ST_CONNECT == spp_state) {
        ...
        bt_cmd_prepare(USER_CTRL_SPP_DISCONNECT, 0, NULL);
    }
}
