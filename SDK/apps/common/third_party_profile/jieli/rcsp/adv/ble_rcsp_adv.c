...
#if (CONFIG_CPU_BR27 || CONFIG_CPU_BR28) && ((RCSP_MODE == RCSP_MODE_SOUNDBOX) || (RCSP_MODE == RCSP_MODE_EARPHONE))
...
int rcsp_make_set_adv_data(void)
{
    ...
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, (void *)mac_addr_buf, 6);
...
static void rcsp_adv_notify_tws_sync_in_irq(void *_data, u16 len, bool rx)
{
    if (rx) {
        ...
        /* printf("%s, %s, %d, adv_notify:%d\n", __FILE__, __FUNCTION__, __LINE__, __this->ble_adv_notify); */
    }
}
...
#if TCFG_USER_TWS_ENABLE
#define TWS_FUNC_ID_RCSP_TWS_ROLE_SW \
	(((u8)('R' + 'C' + 'S' + 'P') << (3 * 8)) | \
	 ((u8)('T' + 'W' + 'S') << (2 * 8)) | \
	 ((u8)('R' + 'O' + 'L' + 'E') << (1 * 8)) | \
	 ((u8)('S' + 'W') << (0 * 8)))
...
static void rcsp_tws_role_sw_in_irq(void *_data, u16 len, bool rx)
{
    if (rx) {
...
REGISTER_TWS_FUNC_STUB(rcsp_tws_role_sw) = {
    .func_id = TWS_FUNC_ID_RCSP_TWS_ROLE_SW,
    .func = rcsp_tws_role_sw_in_irq,
};


extern u8 check_le_pakcet_sent_finish_flag(void);
extern bool rcsp_send_list_is_empty(void);
static u8 g_slave_disconn_try_cnt = 0;
static void adv_master_to_slave_disconn_ble(void *priv)
{
    if (tws_api_get_role() == TWS_ROLE_MASTER) {
        return;
    }
    ...
    y_printf("---%d, %d, %d",rcsp_send_list_is_empty(), check_le_pakcet_sent_finish_flag(), g_slave_disconn_try_cnt);
    // 发送完数据后，或者尝试200次没有发命令成功，则强行关闭新从机的ble模块，并通知新主机开启ble模块
    if ((rcsp_send_list_is_empty() && check_le_pakcet_sent_finish_flag()) || (g_slave_disconn_try_cnt >= 200)) {
        g_slave_disconn_try_cnt = 0;
        y_printf("===========3")
        ble_module_enable(0);
        // 通知新主机开启ble模块
        tws_api_send_data_to_sibling(NULL, 0, TWS_FUNC_ID_RCSP_TWS_ROLE_SW);
    } else {
        g_slave_disconn_try_cnt++;
        sys_timeout_add(NULL, adv_master_to_slave_disconn_ble, 50);
    }
}
#endif
...
void adv_role_switch_handle(void)
{
    ...
    if (tws_api_get_tws_state()) {
        if (bt_rcsp_ble_conn_num() > 0) {
            if (tws_api_get_role() != TWS_ROLE_MASTER) {
                // 原来的主机变成了现在的从机，先发送命令让手机记得回连，关闭ble模块后，tws告诉新主机开启ble模块
                printf("adv_cmd03 %s, %s, %d\n", __FILE__, __FUNCTION__, __LINE__);
                adv_cmd = 0x3;
                // adv_info_device_request(&adv_cmd, sizeof(adv_cmd));
                // 从机关闭ble模块
                adv_master_to_slave_disconn_ble(NULL);
            } else {
                ble_module_enable(1);
        ...
        // if ((bt_rcsp_device_conn_num() < rcsp_max_support_con_dev_num()) && (!bt_rcsp_ble_conn_num())) {
        //     if (tws_api_get_role() == TWS_ROLE_MASTER) {
        //         y_printf("===========1")
        //         ble_module_enable(0);
        //         ble_module_enable(1);
