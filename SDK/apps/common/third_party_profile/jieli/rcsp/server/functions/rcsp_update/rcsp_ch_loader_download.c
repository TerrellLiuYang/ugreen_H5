...
void rcsp_clean_update_hdl_for_end_update(u16 ble_con_handle, u8 *spp_remote_addr)
{
    ...
    if (spp_remote_addr && !memcmp(spp_remote_addr, __this->spp_remote_addr, 6)) {
        __this->ble_con_handle = 0;
        __this->spp_remote_addr = NULL;
    }
...
static u8 g_rcsp_exit_update_mode = 0;

void rcsp_exit_update_mode_enable(void)
{
	g_rcsp_exit_update_mode = 1;
	rcsp_resume();
}

void rcsp_exit_update_mode_disable(void)
{
	g_rcsp_exit_update_mode = 0;
}
...
__RETRY:
	...
	if (0 == g_rcsp_exit_update_mode) {
		__this->data_send_hdl(fp, __this->file_offset, len);
	}

    while (!((0 == __this->state) && (__this->read_len == len))) {
        if (__this->sleep_hdl && get_rcsp_connect_status() && (0 == g_rcsp_exit_update_mode)) {
        ...
        if (!((0 == __this->state) && (__this->read_len == len))) {
            if (g_rcsp_exit_update_mode || retry_cnt++ > RETRY_TIMES) {
                len = (u16) - 1;
...
static u8 update_result_handle(u8 err)
{
    ...
    y_printf("update_result_handle res:%d", res);
    return res;
}
...
static u16 rcsp_f_stop(u8 err)
{
    ...
    y_printf("%s, %d, %x\n", __func__, __LINE__, __this->send_update_status_hdl);
