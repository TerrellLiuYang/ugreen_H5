...
#define rcsp_printf(x, ...)             g_printf("[SYS_RCSP]" x " ", ## __VA_ARGS__)
...
void disconn_device_bef_updata(void);
...
int JL_rcsp_update_cmd_resp(void *priv, u8 OpCode, u8 OpCode_SN, u8 *data, u16 len, u16 ble_con_handle, u8 *spp_remote_addr)
{
    ...
    case JL_OPCODE_INQUIRE_DEVICE_IF_CAN_UPDATE:
        ...
        if (len) {
            ...
            u8 box_vbat = user_charge_info.box_bat_percent &= ~BIT(7);      //去掉充电标志位
            rcsp_printf("l vbat = %d, r vbat = %d, box vabt = %d", user_charge_info.ear_bat_percent_l, user_charge_info.ear_bat_percent_r, box_vbat);
            if (user_charge_info.ear_bat_percent_l < 20 || user_charge_info.ear_bat_percent_r < 20 || box_vbat < 20) {
                rcsp_printf("lowpower, return err!!!\n");
            ...
            rcsp_printf("local_in_box_flag:%d, sibling_in_box_flag:%d", local_in_box_flag, sibling_in_box_flag);
            ...
            if (ear_in_box_status == 0) {
                can_update_flag = UPDATE_FLAG_NO_INBOX;        //暂时用这个错误码，后面跟APP协商加
                goto __user_update_err;
            }
            // 1t2判断是否已有升级任务
            ...
            if (cur_con_dev > 1) {
                ...
                } else if (spp_remote_addr != NULL) {
                    rcsp_printf("spp ota!!!\n");
                    put_buf(spp_remote_addr, 6);
                    local_addr = spp_remote_addr;
                }
                u8 *other_conn_addr = btstack_get_other_dev_addr(local_addr);
                if (other_conn_addr) {
                    rcsp_printf("two dev conn now, disconn other!!!\n");
                    put_buf(other_conn_addr, 6);
                    bt_cmd_prepare_for_addr(other_conn_addr, USER_CTRL_DISCONNECTION_HCI, 0, NULL);
...
__user_update_err:
            y_printf("can_update_flag:%d", can_update_flag);
            if (UPDATE_FLAG_OK != can_update_flag) {
                break;
            }
...
void disconn_device_bef_updata(void)
{
    y_printf("disconn_device_bef_updata");
    u8 spp_working_addr[6];
    u8 nums= bt_get_total_connect_dev();
    if (get_spp_working_addr() != NULL && nums == 2) {
        y_printf("!%d \n",nums);
        u8 *p = get_spp_working_addr();
        memcpy(spp_working_addr,p,6);
        put_buf(spp_working_addr, 6);
        if (memcmp(spp_working_addr,user_phone_info2.addr,6)) {
            user_app_disconn_device(user_phone_info2.addr);
        } else {
            user_app_disconn_device(user_phone_info1.addr);
        } 
    }
}