...
static void tuya_ble_handle_dev_info_req(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    p_buf[87] |= 0x08;
#endif
#if (BT_CAPABILITY_TRANS_SUPPORT)
    p_buf[87] |= 0x40;
#endif
...
static void tuya_ble_handle_bt_state_req(uint8_t *recv_data, uint16_t recv_len)
{
    ...
    extern void tuya_one_click_connect_info_update();
    tuya_one_click_connect_info_update();
...
uint8_t tuya_ble_commData_send(uint16_t cmd, uint32_t ack_sn, uint8_t *data, uint16_t len, uint8_t encryption_mode)
{
    ...
    TUYA_BLE_LOG_INFO("cmd:%x, ble_commData_send len = %d , package_number = %d , protocol version : 0x%02x , error code : 0x%02x", cmd, air_send_packet.encrypt_data_buf_len, package_number, TUYA_BLE_PROTOCOL_VERSION_HIGN, err);
