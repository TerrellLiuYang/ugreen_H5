...
void tws_ota_event_post(u8 event)
{
}
...
static void tws_ota_update_loop(void *priv)
{
    ...
    while (1) {
        ...
        if (sniff_wait_exit_timeout) {
            ...
            dual_bank_passive_update_init(__this->para.fm_crc, __this->para.fm_size, __this->para.max_pkt_len, NULL);
...
int bt_ota_event_handler(struct bt_event *bt)
{
    ...
    case OTA_START_UPDATE_READY:
        log_info("OTA_START_UPDATE_READY:%x %x %d\n", __this->para.fm_crc, __this->para.fm_size, __this->para.max_pkt_len);
