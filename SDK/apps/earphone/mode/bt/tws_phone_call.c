...
extern void phone_check_inband_ring_play_timer(void *priv);
extern u8 get_device_inband_ringtone_flag(void);
extern void phone_income_num_check(void *priv);
extern u8 btstack_get_inband_ringtone_flag_for_addr(u8 *addr);
extern void tws_phone_call_send_cmd(u8 cmd, u8 *btaddr, u8 bt_value, u8 tx_do_action);
extern void bt_api_esco_status(u8 *addr, u8 status);
extern u8 *btstack_get_other_dev_addr(u8 *addr);
enum {
...
void user_call_ring_play_start()
{
...
int bt_phone_income(u8 after_conn, u8 *bt_addr)
{
    ...
    if (tws_api_get_role() == TWS_ROLE_SLAVE) {
        ...
        g_printf("inband_ringtone = 0x%x\n", g_bt_hdl.inband_ringtone);
        g_printf("after_conn = %d\n", after_conn);
        g_printf("bt_stack_get_incoming_call_num() = %d\n", bt_stack_get_incoming_call_num());
        g_bt_hdl.phone_ring_flag = 1;
        g_bt_hdl.phone_income_flag = 1;
        ...
        if (bt_stack_get_incoming_call_num() > 1) {
            ...
            if (g_bt_hdl.inband_ringtone) {
                if (after_conn == 2) { //强制播本地铃声
                    ...
                    u8 *other_addr = NULL;
                    other_addr = btstack_get_other_dev_addr(user_bt_dev_addr.active_addr);
                    u8 active_dev_status = bt_get_call_status_for_addr(user_bt_dev_addr.active_addr);
                    u8 other_dev_status = bt_get_call_status_for_addr(other_addr);
                    g_printf("active_dev_status = %d", active_dev_status);
                    g_printf("other_dev_status = %d", other_dev_status);
                    if (active_dev_status == BT_CALL_ACTIVE && other_dev_status == BT_CALL_INCOMING) {
                        g_printf("ring!!!!!!!!!!!!");
                        // tws_play_ring_file_alone(get_tone_files()->phone_in, SYNC_TONE_PHONE_RING_TIME);
                        // tws_play_ring_file(get_tone_files()->phone_in, SYNC_TONE_PHONE_RING_TIME);
                        sys_timeout_add(NULL, user_call_ring_play_start, 2000);
                    }
                }
            } else {
...
extern void audio_smart_voice_detect_open(u8 model);
...
int bt_phone_esco_play(u8 *bt_addr)
{
    ...
    if (bt_get_call_status() == BT_CALL_ACTIVE) {
        ...
        esco_dump_packet = ESCO_DUMP_PACKET_DEFAULT;
        // y_printf("app_var.call_volume = %d", app_var.call_volume);
        // app_audio_set_volume(APP_AUDIO_STATE_CALL, app_var.call_volume, 1);
    }
...
static int bt_phone_num_to_file_list(const char *file_list[], int max_file_num)
{
    ...
    for (i = 0; num[i] != '\0' && i < max_file_num; i++) {
...
static int bt_phone_status_event_handler(int *msg)
{
    ...
    case BT_STATUS_SCO_STATUS_CHANGE:
        y_printf("BT_STATUS_SCO_STATUS_CHANGE len:%d, type:%d\n",
        ...
        if (bt->value != 0xff) {
            ...
            y_printf("---bt_get_call_vol_for_addr--%d\n", call_vol);
