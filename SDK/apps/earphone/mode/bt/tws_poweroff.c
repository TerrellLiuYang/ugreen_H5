...
{
    y_printf("tws_sync_call_poweroff");
    y_printf("no poweroff!!!");
...
static void sys_auto_shut_down_deal(void *priv)
{
    y_printf("sys_auto_shut_down_deal");
...
{
    y_printf("wait_exit_btstack_flag");
...
{
    y_printf("wait_tws_detach_handler");
    ...
    if (!a2dp_player_runing() && !esco_player_runing()) {
        ...
        if (state & TWS_STA_PHONE_DISCONNECTED) {
            if (state & TWS_STA_SIBLING_CONNECTED) {
                ...
                sys_timer_del(g_bt_detach_timer);
                app_goto_mode(APP_MODE_IDLE, IDLE_MODE_WAIT_POWEROFF);
            } else {
                sys_timer_del(g_bt_detach_timer);
                app_goto_mode(APP_MODE_IDLE, IDLE_MODE_PLAY_POWEROFF);
            }
...
{
    y_printf("sys_enter_soft_poweroff: %d\n", reason);

    if (app_var.goto_poweroff_flag) {
        return;
    }
    percent_save_vm();
    user_local_app_info_save();

    // if ((reason==POWEROFF_NORMAL) || (reason==POWEROFF_NORMAL_TWS)) {
    //     g_printf("user poweroff ui!!!");
    //     led_ui_manager_display(LED_LOCAL, LED_UUID_WHITE_ON_1S, LED_MODE_C);
