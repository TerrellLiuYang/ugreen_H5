...
#if 1
#define bt_tws_log(x, ...)       g_printf("[USER_BT_TWS]" x " ", ## __VA_ARGS__)
#define bt_tws_log_hexdump       g_printf("[USER_BT_TWS]bt_tws_log_hexdump:");\
                                    put_buf
#else
#define bt_tws_log(...)
#define bt_tws_log_hexdump(...)
#endif
...
#define CONFIG_TWS_CHANNEL_SELECT   TCFG_BT_TWS_CHANNEL_SELECT

// extern void user_event_handler(int *data, u16 len);
...
extern void phone_income_num_play();
...
extern void user_enter_pair_mode();
extern void tws_sync_dual_conn_info();
...
u8 role_switch_start = 0; //正在切换标志
u8 tws_monitor_start = 0;//监听开始
...
static void tws_sync_call_fun(int cmd, int err)
{
    ...
    case SYNC_CMD_IRSENSOR_EVENT_FAR:
        ...
        break;
    case SYNC_CMD_5V_DETECET_OFF:
        bt_tws_log("SYNC_CMD_5V_DETECET_OFF\n");
        // charge_set_ldo5v_detect_stop(1);//升级时关闭5V检测
        // charge_module_stop();
        extern USER_VAR user_var;
...
u8 set_channel_by_code_or_res(void)
{
    u8 count = 0;
    char channel = 0;
    char last_channel = 0;
...
int bt_tws_poweron()
{
    ...
    if (err == 0) {
        ...
        bt_tws_log("page_list_empty():%d", page_list_empty());
            //开机无TWS记录，有BT记录，直接TWS配对超时，去回连手机
        if (!page_list_empty()) {
            // app_send_message(APP_MSG_TWS_POWERON_PAIR_TIMEOUT, 0);
            app_send_message(APP_MSG_BT_PAGE_DEVICE, 0);
        } else {
            //开机无TWS记录，无BT记录，就去配对TWS
            app_send_message(APP_MSG_TWS_UNPAIRED, 0);
        }
#endif
...
static void bt_tws_anc_sync(void *_data, u16 len, bool rx)
{
    if (rx) {
        ...
        audio_icsd_adt_state_sync(data[3], data[4]);
#endif /*TCFG_AUDIO_ANC_ACOUSTIC_DETECTOR_EN*/
    }
}
...
extern u8 get_a2dp_spatial_audio_mode(void);
extern void set_a2dp_spatial_audio_mode(u8 mode);
...
int bt_tws_connction_status_event_handler(int *msg)
{
    ...
    case TWS_EVENT_CONNECTED:
        bt_tws_log("TWS_EVENT_CONNECTED");
        ...
        user_send_app_info_to_sibling();
        if (tws_api_get_role() == TWS_ROLE_SLAVE) {
            bt_tws_log("this role is slave, disable adv!!!");
            bt_ble_adv_enable(0);   //连上后从机关广播
        }
        extern u8 soft_ver_local[3];
        u8 data_sync_ver[4] = {CMD_SYNC_VER_INFO, soft_ver_local[0], soft_ver_local[1], soft_ver_local[2]};
        user_send_data_to_sibling(data_sync_ver, sizeof(data_sync_ver));
        //连接上后，将自己的状态给对方即可
        u8 local_find_ear_status = tws_api_get_local_channel()=='L' ? user_var.find_ear_status_l : user_var.find_ear_status_r;
        u8 data_sync_find_ear[2] = {CMD_SYNC_FIND_EAR_STATUS, local_find_ear_status};
        user_send_data_to_sibling(data_sync_find_ear, sizeof(data_sync_find_ear));
        ...
        if (tws_api_get_role() == TWS_ROLE_MASTER) {
            ...
            //同步一下主机的eq模式
            u8 data_set_eq[3] = {CMD_SYNC_SET_EQ_MODE, user_app_info.eq_mode, 0};
    ...
    case TWS_EVENT_CONNECTION_DETACH:
        bt_tws_log("TWS_EVENT_CONNECTION_DETACH");
        /*
         * TWS连接断开
         */
        tws_monitor_start = 0;
    ...
    case TWS_EVENT_PHONE_LINK_DETACH:
        bt_tws_log("TWS_EVENT_PHONE_LINK_DETACH");
        /*
    ...
    case TWS_EVENT_ROLE_SWITCH:
        bt_tws_log("TWS_EVENT_ROLE_SWITCH:%d\n", role);
        bt_tws_log("enter_pairing:%d, total_connect:%d", user_var.enter_pairing_mode, bt_get_total_connect_dev());
        role_switch_start = 0;
        user_app_data_notify(CMD_NOTIFY_BAT_VALUE);
        user_app_data_notify(CMD_NOTIFY_ROLE_SWITCH);
    ...
    case TWS_EVENT_ESCO_ROLE_SWITCH_START:
        bt_tws_log("TWS_EVENT_ESCO_ROLE_SWITCH_START:%d\n", role);
#if TCFG_TWS_POWER_BALANCE_ENABLE
        if (role == TWS_ROLE_SLAVE) {
            esco_recoder_switch(1);
        }
    ...
    case TWS_EVENT_MODE_CHANGE:
        bt_tws_log("TWS_EVENT_MODE_CHANGE : %d", evt->args[0]);
        if (evt->args[0] == 0) {
            tws_sniff_controle_check_enable();
        }
        break;
...
void tws_cancel_state()
{
    ...
    tws_api_cancle_wait_pair();
    tws_api_cancle_create_connection();
    tws_api_detach(TWS_DETACH_BY_POWEROFF, 0);
    gtws.state = 0;
}
...
{
    struct tws_event *evt = (struct tws_event *)priv;
...
// // u8 bt_phone_dec_is_running();
// u8 role_switch_start = 0; //正在切换标志
// static u8 tws_monitor_start = 0;//监听开始

void role_switch(void)
{
    if (!(gtws.state & BT_TWS_SIBLING_CONNECTED)) {
        return;
    }
    ...
    if(role_switch_start){
        puts("*****************role_switch_ing*****************\n");
        return;
    }
    puts("bt_tws_role_switch\n");
    if (gtws.state & BT_TWS_PHONE_CONNECTED){
        role_switch_start = 1;
        if(bt_get_esco_coder_busy_flag()){
            printf("esco tws_api_role_switch\n");
            // test_esco_role_switch(1);
            tws_api_role_switch();
        }else{
            printf("tws_conn_switch_role\n");
            // tws_conn_switch_role();//连接手机时的TWS主从切换
            bt_tws_role_switch();
        }
        return;
    }
...
/**
 * @brief 主从切换处理，单耳在仓或者电量差异切换
 *
 */
void tws_do_role_switch(void)
{
    if(!get_charge_online_flag() && !(get_tws_sibling_bat_persent() & BIT(7))){
        printf("tws_api_auto_role_switch_enable\n");
        if(tws_api_get_role() == TWS_ROLE_MASTER && tws_power_level_cmp()){
            printf("switch role by power balance\n");
            //role_switch();//电量差异切主从
        }
        tws_api_auto_role_switch_enable();
    }
...
static int role_switch_check_timer = 0;

void role_switch_check(void *p);

void role_switch_check_delay(u16 ms)
{
    if(role_switch_check_timer){
        sys_timeout_del(role_switch_check_timer);
        role_switch_check_timer = 0;
    }
    printf("role_switch_check_delay %d\n", ms);
    role_switch_check_timer = sys_timeout_add(NULL, role_switch_check, ms);
}

// void auto_role_switch_ctrl(void)
// {
//     printf("auto_role_switch_ctrl\n");
//     if((get_charge_online_flag() && !(get_tws_sibling_bat_persent() & BIT(7)))
//         || (!get_charge_online_flag() && (get_tws_sibling_bat_persent() & BIT(7))))
//     {
//         printf("tws_api_auto_role_switch_disable\n");
//         tws_api_auto_role_switch_disable();
//     }
//     else if(!get_charge_online_flag() && !(get_tws_sibling_bat_persent() & BIT(7))){
//         printf("tws_api_auto_role_switch_enable\n");
//         tws_api_auto_role_switch_enable();
//     }
// }
...
void role_switch_check(void *p)
{
    ...
    printf("role_switch_check");
    if(get_tws_phone_connect_state() == 0 || get_tws_sibling_connect_state() == 0){
        puts("phone or tws disconnected\n");
        return;
    }

    if (bt_is_sniff_close() || !tws_monitor_start) {//退出sniff并且监听开始再切换
        printf("*****************sniff state :%d or not monitor :%d, wait*****************\n", bt_is_sniff_close(), tws_monitor_start);
        if(tws_api_get_role() == TWS_ROLE_MASTER){
            if(tws_monitor_start){
                // user_send_cmd_prepare(USER_CTRL_ALL_SNIFF_EXIT, 0, NULL);
                bt_cmd_prepare(USER_CTRL_ALL_SNIFF_EXIT, 0, NULL);
            }
            role_switch_check_delay(500);
        }
    }
    else{
        tws_do_role_switch();
