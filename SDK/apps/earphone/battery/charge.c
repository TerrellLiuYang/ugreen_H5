#include "app_config.h"
#include "asm/charge.h"
#include "asm/power_interface.h"
#include "system/event.h"
#include "system/includes.h"
#include "app_action.h"
#include "asm/wdt.h"
#include "app_power_manage.h"
#include "app_chargestore.h"
#include "btstack/avctp_user.h"
#include "app_action.h"
#include "app_main.h"
#include "bt_tws.h"
#include "usb/otg.h"
#include "bt_common.h"
#include "battery_manager.h"
#include "app_charge.h"

#define LOG_TAG_CONST       APP_CHARGE
#define LOG_TAG             "[APP_CHARGE]"
#define LOG_ERROR_ENABLE
#define LOG_DEBUG_ENABLE
#define LOG_INFO_ENABLE
/* #define LOG_DUMP_ENABLE */
#define LOG_CLI_ENABLE
#include "debug.h"

#if 1
#define charge_log(x, ...)       g_printf("[USER_CHARGE]" x " ", ## __VA_ARGS__)
#define charge_log_hexdump       g_printf("[USER_CHARGE]charge_log_hexdump:");\
                                    put_buf
#else
#define charge_log(...)
#define charge_log_hexdump(...)
#endif

extern USER_VAR user_var;

void inout_box_deal(u8 inout);
void tws_inout_box_deal();

extern u32 dual_bank_update_exist_flag_get(void);
extern u32 classic_update_task_exist_flag_get(void);

#if TCFG_CHARGE_ENABLE

static u8 charge_full_flag = 0;


u8 get_charge_full_flag(void)
{
    return charge_full_flag;
}

static void charge_start_deal(void)
{
    log_info("%s\n", __FUNCTION__);

    power_set_mode(PWR_LDO15);

    batmgr_send_msg(BAT_MSG_CHARGE_START, 0);
}

static void charge_full_deal(void)
{
    const struct app_charge_handler *handler;

    log_info("%s\n", __func__);

    charge_full_flag = 1;
    charge_close();

    for_each_app_charge_handler(handler) {
        handler->handler(CHARGE_EVENT_CHARGE_FULL, 0);
    }

    if (get_charge_poweron_en() == 0) {
        /* power_set_soft_poweroff(); */
#if (!TCFG_CHARGESTORE_ENABLE)
        vbat_timer_delete();
#endif
    } else {
        batmgr_send_msg(BAT_MSG_CHARGE_FULL, 0);
    }
}

static void charge_close_deal(void)
{
    log_info("%s\n", __FUNCTION__);

    power_set_mode(TCFG_LOWPOWER_POWER_SEL);

    batmgr_send_msg(BAT_MSG_CHARGE_CLOSE, 0);
}


void app_charge_power_off_keep_mode()
{
    //兼容一些充电仓5v输出慢的时候会导致无法充电的问题
    if (get_lvcmp_det()) {
        log_info("...charge ing...\n");
        cpu_reset();
    }

    log_info("get_charge_online_flag:%d %d\n", get_charge_online_flag(), get_ldo5v_online_hw());
    if (get_ldo5v_online_hw() && get_charge_online_flag()) {
        power_set_soft_poweroff();
    } else {
        charge_check_and_set_pinr(1);
#if TCFG_CHARGE_OFF_POWERON_EN
        cpu_reset();
#else
        power_set_soft_poweroff();
#endif
    }
}

static void ldo5v_keep_softoff(void)
{
    log_info("%s\n", __func__);

    if (!app_in_mode(APP_MODE_IDLE)) {
        sys_enter_soft_poweroff(POWEROFF_POWER_KEEP);
    } else {
        app_charge_power_off_keep_mode();
    }
}

/*ldoin电压大于拔出电压(0.6V左右)且小于VBat电压时调用该函数进行一些错误提示或其他处理*/
void ldo5v_keep_deal(void)
{
    int abandon = 0;
    const struct app_charge_handler *handler;

    log_info("%s\n", __func__);

    charge_close();

    //插入交换
    batmgr_send_msg(POWER_EVENT_POWER_CHANGE, 0);

    charge_check_and_set_pinr(0);

    for_each_app_charge_handler(handler) {
        abandon += handler->handler(CHARGE_EVENT_LDO5V_KEEP, 0);
    }

#if defined(TCFG_CHARGE_KEEP_UPDATA) && TCFG_CHARGE_KEEP_UPDATA
    //升级过程中,不执行关机操作
    if (dual_bank_update_exist_flag_get() || classic_update_task_exist_flag_get()) {
        return;
    }
#endif

    if (get_charge_poweron_en() == 0) {
        if (abandon == 0) {
            ldo5v_keep_softoff();
        }
    } else {
        batmgr_send_msg(BAT_MSG_CHARGE_ERR, 0);
    }
}

void charge_ldo5v_in_deal(void)
{
    int abandon = 0;
    const struct app_charge_handler *handler;

    log_info("%s\n", __FUNCTION__);

    //插入交换
    batmgr_send_msg(POWER_EVENT_POWER_CHANGE, 0);

    charge_full_flag = 0;

    charge_check_and_set_pinr(0);
    // if (get_jl_update_flag()){
    if(user_var.is_ota_mode){
        return;
    }
    for_each_app_charge_handler(handler) {
        abandon += handler->handler(CHARGE_EVENT_LDO5V_IN, 0);
    }

#if defined(TCFG_CHARGE_KEEP_UPDATA) && TCFG_CHARGE_KEEP_UPDATA
    //升级过程中,不执行充电插入关机流程
    if (dual_bank_update_exist_flag_get() || classic_update_task_exist_flag_get()) {
        return;
    }
#endif

    if (get_charge_poweron_en() == 0) {
        if (!app_in_mode(APP_MODE_IDLE)) {
            if (abandon == 0) {
                y_printf("charge_ldo5v_in_deal powerreset\n");
                sys_enter_soft_poweroff(POWEROFF_RESET);
            }
        } else {
            charge_start();
            wdt_init(WDT_32S);
            log_info("set wdt to 32s!\n");
            goto _check_reset;
        }
    } else {
        charge_start();
        goto _check_reset;
    }
    return;

_check_reset:
    //防止耳机低电时,插拔充电有几率出现关机不充电问题
    if (app_var.goto_poweroff_flag) {
        cpu_reset();
    }
}

void charge_ldo5v_off_deal(void)
{
    int abandon = 0;
    int off_type = LDO5V_OFF_TYPE_NORMAL_ON;
    u8 chargestore_power_status = 1;
    bool lowpower_flag = FALSE, is_bt_mode, is_idle_mode;
    const struct app_charge_handler *handler;

    log_info("%s\n", __FUNCTION__);

    //拨出交换
    batmgr_send_msg(POWER_EVENT_POWER_CHANGE, 0);

    percent_save_vm();

    charge_full_flag = 0;

    charge_close();

    batmgr_send_msg(BAT_MSG_CHARGE_LDO5V_OFF, 0);

    charge_check_and_set_pinr(1);

#if TCFG_CHARGESTORE_ENABLE
    chargestore_power_status = chargestore_get_power_status();
#endif
#if TCFG_SYS_LVD_EN
    lowpower_flag = get_vbat_need_shutdown();
#endif
    is_bt_mode = app_in_mode(APP_MODE_BT);
    is_idle_mode = app_in_mode(APP_MODE_IDLE);

    if ((get_charge_poweron_en() == 0)) {
        wdt_init(WDT_4S);
        log_info("set wdt to 4s!\n");
        if (chargestore_power_status) {
#if TCFG_CHARGE_OFF_POWERON_EN
            log_info("ldo5v off,task switch to BT\n");
            if (app_var.goto_poweroff_flag == 0) {
                if (!is_bt_mode) {
                    if (lowpower_flag == FALSE) {
                        off_type = LDO5V_OFF_TYPE_NORMAL_ON;//正常拔出开机
                    } else {
                        log_info("ldo5v off,lowpower,need enter softpoweroff\n");
                        user_set_cur_bat_st(2);
                        off_type = LDO5V_OFF_TYPE_LOWPOWER_OFF;//拔出低电关机
                    }
                }
            }
#else //TCFG_CHARGE_OFF_POWERON_EN
            log_info("ldo5v off,enter softpoweroff\n");
            off_type = LDO5V_OFF_TYPE_NORMAL_OFF;//正常拔出关机
#endif
        } else {
            log_info("ldo5v off,enter softpoweroff\n");
            off_type = LDO5V_OFF_TYPE_CHARGESTORE_OFF;
        }
    } else {
        if (is_idle_mode && (app_var.goto_poweroff_flag == 0)) {
#if TCFG_CHARGE_OFF_POWERON_EN
            log_info("ldo5v off,task switch to BT\n");
            if (lowpower_flag == FALSE) {
                off_type = LDO5V_OFF_TYPE_NORMAL_ON;//正常拔出开机
            } else {
                log_info("ldo5v off,lowpower,need enter softpoweroff\n");
                user_set_cur_bat_st(2);
                off_type = LDO5V_OFF_TYPE_LOWPOWER_OFF;//拔出低电关机
            }
#else
            off_type = LDO5V_OFF_TYPE_NORMAL_OFF;//正常拔出关机
#endif
        }
    }

    if (user_var.cmd_enter_poweroff) {
        log_info("cmd enter poweroff!!!\n");
        off_type = LDO5V_OFF_TYPE_NORMAL_OFF;//正常拔出关机
    }
    
    for_each_app_charge_handler(handler) {
        abandon += handler->handler(CHARGE_EVENT_LDO5V_OFF, off_type);
    }

    if (abandon) {
        return;
    }

    switch (off_type) {
    case LDO5V_OFF_TYPE_NORMAL_OFF:
    case LDO5V_OFF_TYPE_LOWPOWER_OFF:
        power_set_soft_poweroff();
        break;
    case LDO5V_OFF_TYPE_CHARGESTORE_OFF:
        if (is_bt_mode) {
            sys_enter_soft_poweroff(POWEROFF_NORMAL);
        } else {
            power_set_soft_poweroff();
        }
        break;
    case LDO5V_OFF_TYPE_NORMAL_ON:
        // app_var.play_poweron_tone = 0;
        app_var.play_poweron_tone = 1;
        app_goto_mode(APP_MODE_BT, 0);
        break;
    }
}

extern void user_voice_mute(u8 en);
static int app_charge_event_handler(int *msg)
{
    int ret = false;
    u8 otg_status = 0;

    switch (msg[0]) {
    case CHARGE_EVENT_CHARGE_START:
        charge_log("CHARGE_EVENT_CHARGE_START");
        charge_start_deal();
        break;
    case CHARGE_EVENT_CHARGE_CLOSE:
        charge_log("CHARGE_EVENT_CHARGE_CLOSE");
        charge_close_deal();
        break;
    case CHARGE_EVENT_CHARGE_FULL:
        charge_log("CHARGE_EVENT_CHARGE_FULL");
        charge_full_deal();
        break;
    case CHARGE_EVENT_LDO5V_KEEP:
        charge_log("CHARGE_EVENT_LDO5V_KEEP");
        user_voice_mute(1);
        if (user_var.is_ota_mode==0)
        {
            inout_box_deal(1);
        }
        ldo5v_keep_deal();
        break;
    case CHARGE_EVENT_LDO5V_IN:
        charge_log("CHARGE_EVENT_LDO5V_IN");
#if ((TCFG_OTG_MODE & OTG_SLAVE_MODE) && (TCFG_OTG_MODE & OTG_CHARGE_MODE))
        while (1) {
            otg_status = usb_otg_online(0);
            if (otg_status != IDLE_MODE) {
                break;
            }
            os_time_dly(2);
        }
        if (otg_status == SLAVE_MODE) {
            set_charge_poweron_en(1);
        }
#endif

        if (get_charge_poweron_en() || (otg_status != SLAVE_MODE)) {
            if (user_var.is_ota_mode==0)
            {
                charge_ldo5v_in_deal();
            }
        }
        break;
    case CHARGE_EVENT_LDO5V_OFF:
        charge_log("CHARGE_EVENT_LDO5V_OFF");
        // user_set_box_charge_status(0xff);
        user_set_local_box_charge_status(0xff);
        //SPP缓存可能满了发不出去，发多几包0xFF
        user_sync_box_charge_status(0xff);
        charge_log("user_var.is_ota_mode = %d", user_var.is_ota_mode);
        if (!user_var.is_ota_mode) {
            user_voice_mute(0);
            inout_box_deal(0);
        } 
        // user_voice_mute(0);
        // if (get_jl_update_flag()==0)
        // {
        //     inout_box_deal(0);
        // }                    
#if ((TCFG_OTG_MODE & OTG_SLAVE_MODE) && (TCFG_OTG_MODE & OTG_CHARGE_MODE))
        while (1) {
            otg_status = usb_otg_online(0);
            if (otg_status != IDLE_MODE) {
                break;
            }
            os_time_dly(2);
        }
#endif
        if (get_charge_poweron_en() || (otg_status != SLAVE_MODE)) {
            charge_ldo5v_off_deal();
        }
        break;
    default:
        break;
    }

    return ret;
}

APP_MSG_HANDLER(bat_charge_msg_entry) = {
    .owner      = 0xff,
    .from       = MSG_FROM_BATTERY,
    .handler    = app_charge_event_handler,
};

#else

u8 get_charge_full_flag(void)
{
    return 0;
}

#endif



/*************************************************************/
//   出入仓主从切换，在这里进行
/*************************************************************/
extern bool bt_is_sniff_close(void);
u8 role_switch_after_exit_sniff = 0;

void inout_box_deal(u8 inout)
{
    charge_log("inout_box_deal");
    // u8 tws_bat = get_tws_sibling_bat_level();
    if (bt_get_total_connect_dev() == 0) {
        charge_log("no phone connect, return!!!");
        return;
    }
    u8 tws_bat = get_tws_sibling_bat_persent();
    charge_log("%s, role:%d, self:%d, tws:%d, inout:%d", __func__, tws_api_get_role(), get_charge_online_flag(), tws_bat, inout);

    role_switch_after_exit_sniff = 0;
    if (get_bt_tws_connect_status()) {
        charge_log("tws is connected!!!");
        if (get_charge_online_flag() && tws_bat >> 7) { //两个都在仓内
            charge_log("two ear in the box!!!");
            tws_api_auto_role_switch_enable();
            //对比样机两个在仓不用切到手机
            // if (bt_get_esco_coder_busy_flag()) {
            //     bt_cmd_prepare(USER_CTRL_DISCONN_SCO, 0, NULL);
            // } else {
            //     bt_cmd_prepare(USER_CTRL_AVCTP_OPID_PAUSE, 0, NULL);
            // }
        } else if (!get_charge_online_flag() && !(tws_bat >> 7)) { //两个都在仓外
            charge_log("no ear in the box!!!");
            tws_api_auto_role_switch_enable();
            user_var.role_switch_disable = 0;
            //对比样机两个在仓不用切到手机
            // if (bt_get_call_status() == BT_CALL_ACTIVE) {
            //     bt_cmd_prepare(USER_CTRL_CONN_SCO, 0, NULL);
            // }
        } else { //只有一边在仓里
            charge_log("one ear in the box!!!");
            tws_api_auto_role_switch_disable();
            user_var.role_switch_disable = 1;
            if (get_charge_online_flag() && tws_api_get_role() == TWS_ROLE_MASTER) {
                if (bt_get_esco_coder_busy_flag()) { //通话调用通话的主从切换接口
                    tws_api_role_switch();
                } else if (!bt_is_sniff_close()) { //没有进入sniff状态，直接调用主从切换接口
                    // clear_sniff_cnt(); //清一下sniff的计数，防止主从切换没有完成前进入sniff
                    bt_clear_sniff_cnt();
                    tws_api_role_switch();
                } else if (bt_get_total_connect_dev()) { //进入了sniff状态，先退出sniff，等退出sniff再主从切换
                    bt_cmd_prepare(USER_CTRL_ALL_SNIFF_EXIT, 0, NULL);
                    role_switch_after_exit_sniff = 1;
                } else { //没有连接手机，调用对耳的主从切换接口
                    bt_tws_role_switch();
                }
            }
            //对比样机两个在仓不用切到手机
            // if(!inout && bt_get_call_status() == BT_CALL_ACTIVE) {
            //     bt_cmd_prepare(USER_CTRL_CONN_SCO, 0, NULL);
            // }
        }
    } else {
        charge_log("tws no connected!!!");
    }
}

//对耳出入仓处理函数
void tws_inout_box_deal()
{
    charge_log("tws_inout_box_deal");
    if(user_var.is_ota_mode){
        return;
    }
    static u8 tws_last_charge = 0xFF;
    u8 user_tws_sibling_bat_percent_level = get_tws_sibling_bat_persent();
    charge_log("user_tws_sibling_bat_percent_level = %d", user_tws_sibling_bat_percent_level);
    if (user_tws_sibling_bat_percent_level != 0xFF) {
        if (tws_last_charge == 0xFF) { //对耳连接上
            inout_box_deal(user_tws_sibling_bat_percent_level >> 7);
        } else if(tws_last_charge & 0x80 && !(user_tws_sibling_bat_percent_level >> 7)) { //对方耳机出仓
            inout_box_deal(0);
        } else if(!(tws_last_charge & 0x80) && user_tws_sibling_bat_percent_level >> 7) { //对方耳机入仓
            inout_box_deal(1);
        }
    }
    tws_last_charge = user_tws_sibling_bat_percent_level;
}

/*************************************************************/





