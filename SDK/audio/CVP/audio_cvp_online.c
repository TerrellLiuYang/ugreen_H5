...
int aec_cfg_online_update(int root_cmd, void *packet)
{
    ...
    if (id >= ENC_Process_MaxFreq) {
        ...
        case GENERAL_MIC:
            ...
            audio_adc_file_set_gain(0, cfg->val_int);
#if TCFG_AUDIO_DUAL_MIC_ENABLE
#if (TCFG_AUDIO_DMS_SEL == DMS_NORMAL)
            audio_adc_file_set_gain(1, cfg->val_int);
#endif/*TCFG_AUDIO_DMS_SEL*/
#endif/*TCFG_AUDIO_DUAL_MIC_ENABLE*/
            update = 0;
            break;
...
int aec_cfg_online_init()
{
...
#if TCFG_AUDIO_DUAL_MIC_ENABLE
#if (TCFG_AUDIO_DMS_SEL == DMS_NORMAL)
    aec_update->cfg.mic_again = audio_adc_file_get_gain(0);
    ...
    /* } */
    aec_update->cfg.mic_again = audio_adc_file_get_gain(0);
    aec_update->cfg.mic1_again = audio_adc_file_get_gain(1);
#endif/*TCFG_AUDIO_DMS_SEL*/
#else/*SINGLE MIC*/
    aec_update->cfg.mic_again = audio_adc_file_get_gain(0);
    aec_update->cfg.dac_again = app_var.aec_dac_gain;//dac增益获取接口
