...
int audio_dec_app_audio_state_switch(struct audio_dec_app_hdl *dec, u32 flag)
{
    ...
    if (need_set_audio) {
        if (flag == AUDIO_DEC_FILE_FLAG_AUDIO_STATE_MUSIC) {
            ...
            app_audio_state_switch(APP_AUDIO_STATE_WTONE, app_audio_volume_max_query(SysVol_TONE), NULL);
#else
            app_audio_state_switch(APP_AUDIO_STATE_WTONE, app_audio_volume_max_query(SysVol_TONE), NULL);
#endif
        }
...
void audio_dec_sine_test(void)
{
    ...
    /* hdl = audio_dec_sine_app_create(FLASH_RES_PATH"tone/vol_max.sin", 1); */
