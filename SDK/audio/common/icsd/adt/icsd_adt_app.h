...
enum {
    ANC_WIND_NOISE_WEAK_LVL = 1,    //弱风强等级
    ANC_WIND_NOISE_MIDDLE_LVL,  //中风强等级
    ANC_WIND_NOISE_STRONG_LVL,  //强风等级
};
...
/*同步tws配对时，同步adt的状态*/
void audio_icsd_adt_state_sync(u8 adt_mode, u8 speak_to_chat_state);

/*检测到讲话状态同步*/
void set_speak_to_chat_voice_state(u8 state);
void audio_speak_to_chat_voice_state_sync(void);

int audio_icsd_adt_open();
int audio_icsd_adt_sync_open();
int audio_icsd_adt_close();
/*关闭所有模块*/
