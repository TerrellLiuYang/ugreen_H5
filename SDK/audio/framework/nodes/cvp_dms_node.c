...
#if ((defined TCFG_AUDIO_CVP_DMS_ANS_MODE) || (defined TCFG_AUDIO_CVP_DMS_DNS_MODE) \
 || (defined TCFG_AUDIO_CVP_DMS_FLEXIBLE_ANS_MODE) || (defined TCFG_AUDIO_CVP_DMS_FLEXIBLE_DNS_MODE))
...
struct cvp_node_hdl {
    char name[16];
#if (defined TCFG_AUDIO_CVP_DMS_ANS_MODE) || (defined TCFG_AUDIO_CVP_DMS_DNS_MODE)
    AEC_DMS_CONFIG online_cfg;
#else
    DMS_FLEXIBLE_CONFIG online_cfg;
#endif
...
int cvp_param_cfg_read(void)
{
...
#if (defined TCFG_AUDIO_CVP_DMS_ANS_MODE)
    len = jlstream_read_node_data(NODE_UUID_CVP_DMS_ANS, 0XFF, (u8 *)&ncfg);
#elif (defined TCFG_AUDIO_CVP_DMS_DNS_MODE)
    len = jlstream_read_node_data(NODE_UUID_CVP_DMS_DNS, 0XFF, (u8 *)&ncfg);
#elif (defined TCFG_AUDIO_CVP_DMS_FLEXIBLE_ANS_MODE)
    len = jlstream_read_node_data(NODE_UUID_CVP_DMS_FLEXIBLE_ANS, 0XFF, (u8 *)&ncfg);
#else /*TCFG_AUDIO_CVP_DMS_FLEXIBLE_DNS_MODE*/
    len = jlstream_read_node_data(NODE_UUID_CVP_DMS_FLEXIBLE_DNS, 0XFF, (u8 *)&ncfg);
#endif
    if (len != sizeof(ncfg)) {
    ...
    printf(" %s len %d, sizeof(global_cvp_cfg) %ld\n", __func__,  len, sizeof(global_cvp_cfg));
...
#if (defined TCFG_AUDIO_CVP_DMS_ANS_MODE) || (defined TCFG_AUDIO_CVP_DMS_DNS_MODE)
        aec_dms_cfg_update(&hdl->online_cfg);
#else
        aec_dms_flexible_cfg_update(&hdl->online_cfg);
#endif
        ret = true;
    }
    return ret;
...
#ifdef TCFG_AUDIO_CVP_DMS_ANS_MODE
    .uuid       = NODE_UUID_CVP_DMS_ANS,
#elif (defined TCFG_AUDIO_CVP_DMS_DNS_MODE)
    .uuid       = NODE_UUID_CVP_DMS_DNS,
#elif (defined TCFG_AUDIO_CVP_DMS_FLEXIBLE_ANS_MODE)
