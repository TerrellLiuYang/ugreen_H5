...
static int jl_kws_adc_mic_open(void)
{
    ...
    audio_adc_file_init();
    adc_file_mic_open(&(__kws_audio->kws_adc->mic_ch), TCFG_KWS_MIC_CHA_SELECT);
#else
    ...
    mic_param.mic_dcc       = 8;
    audio_adc_mic_open(&(__kws_audio->kws_adc->mic_ch), TCFG_KWS_MIC_CHA_SELECT, &adc_hdl, &mic_param);
    audio_adc_mic_set_gain(&(__kws_audio->kws_adc->mic_ch), TCFG_KWS_MIC_CHA_SELECT, mic_gain);
    /* audio_adc_mic_0dB_en(TCFG_KWS_MIC_CHA_SELECT, 1); */
#endif
...
int jl_kws_audio_get_data(void *buf, u32 len)
{
    ...
    if (data_len >= len) {
        ...
        os_sem_set(&(__kws_audio->rx_sem), 0);
#if TCFG_JL_KWS_AUDIO_DATA_FROM_EXTERN
        ret = os_sem_pend(&(__kws_audio->rx_sem), 500);
        if (ret == OS_TIMEOUT) {
