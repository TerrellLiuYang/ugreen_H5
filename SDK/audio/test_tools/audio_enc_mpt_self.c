...
#define ENC_MPT_SELF_TASK_NAME  "enc_mpt_sel"
...
{
    u8 *data_home;
    enc_freres_file_head_t *file_head = (enc_freres_file_head_t *)file->dat;
    ...
    (int)hdl->bulk[cnt].out, (int)data_home, AUDIO_ENC_MPT_FRERES_POINT * (int)sizeof(short));
    //len(byte)
    hdl->bulk[cnt].len = 0;
...
void audio_enc_mpt_fre_response_start(u16 ch)
{
    ...
    task_create(audio_enc_mpt_fre_resonpse_task, NULL, ENC_MPT_SELF_TASK_NAME);
    ...
    encmpt_log("det_cnt %d, list bulk size %d\n", det_cnt, (int)sizeof(enc_freres_bulk) * det_cnt);
...
#if AUDIO_ENC_MPT_FRERES_ASYNC
    os_taskq_post_msg(ENC_MPT_SELF_TASK_NAME, 2, ENC_FRE_RESPONE_MSG_RUN, id);
#else
...
#if AUDIO_ENC_MPT_FRERES_ASYNC
    task_kill(ENC_MPT_SELF_TASK_NAME);
#endif/*AUDIO_ENC_MPT_FRERES_ASYNC*/
    enc_freres_bulk *bulk;
    list_for_each_entry(bulk, &hdl->head, entry) {
        free(bulk->in);
