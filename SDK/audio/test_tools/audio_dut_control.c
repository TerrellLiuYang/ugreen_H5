...
int cvp_dut_bypass_mic_ch_sel(u8 mic_num)
{
    if (audio_dut_hdl) {
        switch (mic_num) {
        case A_MIC0:
            audio_dut_hdl->mic_ch_sel = AUDIO_ADC_MIC_0;
            break;
        case A_MIC1:
            audio_dut_hdl->mic_ch_sel = AUDIO_ADC_MIC_1;
            break;
#if ANC_CHIP_VERSION != ANC_VERSION_BR36
        case A_MIC2:
            audio_dut_hdl->mic_ch_sel = AUDIO_ADC_MIC_2;
            break;
        case A_MIC3:
            audio_dut_hdl->mic_ch_sel = AUDIO_ADC_MIC_3;
            break;
#endif
        default:
            break;
...
//JL自研算法指令API
        ...
        ret = (ret == -1) ? AUDIO_DUT_ACK_SUCCESS : ret;
    }
    return ret;
}

//len = 参数数据长度(不包括cmd)
int audio_dut_new_tx_packet(u8 cmd, u8 *data, int len)
{
    if (audio_dut_hdl) {
    ...
    if (audio_dut_hdl) {
        ...
        if (head->magic == AUDIO_DUT_NEW_SPP_MAGIC) {
            if (head->crc == crc || head->crc == 0x1) {
                ret = audio_dut_new_event_deal(&head->packet);
