...
#define dms_spp_log(x, ...)       g_printf("[USER_DMS_SPP]" x " ", ## __VA_ARGS__)
#else
#define dms_spp_log(...)
#endif
...
{
    dms_spp_log("dms_spp_event_deal");
    if (dms_spp_hdl) {
        switch (dat[3]) {
...
static int dms_app_online_parse(u8 *packet, u8 size, u8 *ext_data, u16 ext_size)
{
    dms_spp_log("dms_app_online_parse");
    if (dms_spp_hdl) {
...
{
    dms_spp_log("dms_spp_event_deal_new");
    u16 len;
    int ret = 0;
    /* 数据长度 */
    ...
    if (dms_spp_hdl) {
        ...
        case CVP_ENC_MIC_DIFF_CMP_SET:
            dms_spp_log("CVP_ENC_MIC_DIFF_CMP_SET\n");
            float diff;
        ...
        case CVP_ENC_MIC_DIFF_CMP_EN:
            dms_spp_log("CVP_ENC_MIC_DIFF_CMP_EN\n");
            u8 en;
            memcpy(&en, dat + 3, len);
...
{
    dms_spp_log("dms_spp_tx_packet_new");
    if (dms_spp_hdl) {
...
int dms_spp_rx_packet_new(u8 *dat, u8 len)
{
    dms_spp_log("dms_spp_rx_packet_new");
    u16 crc = 0;
    int ret = 0;
