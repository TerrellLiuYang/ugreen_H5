...
static void plnk_info_dump()
{
    ...
    plnk_printf("CLK_CON = 0x%x", JL_CLOCK->CLK_CON0);
    plnk_printf("PLNK_CON1 = 0x%x", JL_PLNK->CON1);
    plnk_printf("PLNK_LEN = 0x%x", JL_PLNK->LEN);
    plnk_printf("PLNK_ADR = 0x%x", JL_PLNK->ADR);
    plnk_printf("PLNK_SMR = %d", JL_PLNK->SMR);
    plnk_printf("PLNK_DOR = %d", JL_PLNK->DOR);
}

static void plnk_sr_set(u16 sr)
{
    u32 plnk_dor = __this->sclk_fre / sr - 1;
    u32 plnk_smr = PLNK_CLK / __this->sclk_fre - 1;
    plnk_printf("PLNK_SR = %d, dor = %d, smr = %d\n", sr, plnk_dor, plnk_smr);
    PLNK_DOR_SET(plnk_dor);
    PLNK_SMR_SET(plnk_smr);
}
...
void *plnk_init(void *hw_plink)
{
    ...
    PLNK_CH0_DFDLY(m - 1);
    PLNK_CH1_DFDLY(m - 1);
    PLNK_CH0_ORDER(3 - order);
    PLNK_CH1_ORDER(3 - order);

    plnk_set_shift_scale(order, m);
