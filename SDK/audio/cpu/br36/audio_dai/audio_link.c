...
{
    gpio_set_mode(IO_PORT_SPILT(gpio), PORT_INPUT_PULLUP_10K);
}
...
void alink_channel_init(u8 ch_idx, u8 dir, void (*handle)(s16 *buf, u32 len))
{
    ...
    /* p_alink_parm->ch_cfg[ch_idx].buf = zalloc(1024*4); */
    u32 *buf_addr;
    ...
    *buf_addr = (u32)(p_alink_parm->ch_cfg[ch_idx].buf);
    ...
    if (p_alink_parm->ch_cfg[ch_idx].dir == ALINK_DIR_RX) {
        gpio_set_mode(IO_PORT_SPILT(p_alink_parm->ch_cfg[ch_idx].data_io), PORT_INPUT_PULLUP_10K);
        ...
        gpio_set_fun_output_port(p_alink_parm->ch_cfg[ch_idx].data_io, FO_ALNK0_DAT[ch_idx], 1, 1, 0);
...
{
    gpio_set_mode(IO_PORT_SPILT(p_alink_parm->ch_cfg[ch_idx].data_io), PORT_HIGHZ);
